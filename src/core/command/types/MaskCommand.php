...
class MaskCommand extends Command {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        if(!in_array($args[0], ["list", "charm"])){
            ...
            foreach(Urbis::getInstance()->getMaskManager()->getMasks() as $damage => $mask){
                $name = $mask->getName();
                ...
                if(strtolower($args[0]) == $cmdName){
                    ...
                    $target->sendMessage("§l§a»§r §7You have gotten a §b" . $name . "§7.");
                    break;
                }
            }
            return;
                ...
                if(isset($args[1])){
                    if($args[1] == "all"){
                        foreach(Server::getInstance()->getOnlinePlayers() as $player) {
                            $name = $sender->getName();
                            $player->sendMessage("§l§a»§r §7$name has given a Mask Charm to everyone that's online!");
                        ...
                        if($player instanceof CorePlayer){
                            $player->sendMessage("§l§a»§r §7You have successfully been given a Mask Charm!");
                            $i = self::getMaskCharmItem($amt);
                            $player->getInventory()->addItem($i);
                        }else{
                            $sender->sendMessage("§l§c»§r §7Player cannot be found.");
                        }
                    }
                }else{
                    if(!$sender instanceof CorePlayer){
                        $sender->sendMessage("§l§c»§r §7Console are not allow to do this");
                        return;
                    }
                    $sender->sendMessage("§l§a»§r §7You have successfully given a Mask Charm!");
                    $i = self::getMaskCharmItem($amt);
    ...
    public static function getMaskCharmItem($amt = 1): Item{
        ...
        $i->setCustomName("§l§bMask§r §7Charm§r");
        $i->setLore([
