...
class WildCommand extends Command {
    ...
    protected function findLocation(Level $level, int $loops = 0): Position {
        if($loops > 10) {
            $x = $level->getSpawnLocation()->getFloorX() + mt_rand(1000, 5000);
            $y = 100;
            $z = $level->getSpawnLocation()->getFloorZ() + mt_rand(1000, 5000);
            return new Position($x, $y, $z, $level);
        }
        $x = mt_rand($level->getSpawnLocation()->getFloorX() + 100, 3700);
        $z = mt_rand($level->getSpawnLocation()->getFloorX() + 100, 3700);
