...
class ClearlagCommand extends Command {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        if(!isset($args[0])){
            $sender->sendMessage("§l§6»§r §7Usage: /clearlag (check/clear/killmobs/clearall)");
            return;
        }
        switch($args[0]){
            case "clear":
                $sender->sendMessage("§l§a»§r §7Removed " . $this->removeEntities() . " entities.");
                break;
            case "check":
            case "count":
                $c = $this->getEntityCount();
                $sender->sendMessage("§l§6»§r §7There are " . $c[0] . " players, " . $c[1] . " mobs, and " . $c[2] . " entities.");
                break;
            case "killmobs":
                $sender->sendMessage("§l§a»§r §7Removed " . $this->removeMobs() . " mobs.");
                break;

            case "clearall":
            case "all":
                $sender->sendMessage("§l§a»§r §7Removed " . ($d = $this->removeMobs()) . " mob" . ($d == 1 ? "" : "s") . " and " . ($d = $this->removeEntities()) . " entit" . ($d == 1 ? "y" : "ies") . ".");
                break;
        }
    }
