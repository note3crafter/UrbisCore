...
class CustomPotionCommand extends Command {
    ...
    public function __construct() {
        parent::__construct("custompotion", "Custom potions... yup!", "/custompotion <player> <type>");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        if(!$sender instanceof UrbislPlayer or !$sender->hasPermission("cryptic.custompotions")) {
            $sender->sendMessage(Translation::getMessage("noPermission"));
            return;
        }
        if(isset($args[0])){
            if(!$player = $sender->getServer()->getPlayer($args[0])){
                $sender->sendMessage("§l§c»§r §7That player does not exist!§r");
                return;
            }
            $name = $player->getName();
            if(isset($args[1])){
                switch($args[1]){
                    case "raiding":
                        $sender->sendMessage("§l§a»§r §7You have given " . $name . " a §l§cRaiding Elixir§r§7.§r");
                        ...
                        break;
                    case "pvp":
                        $sender->sendMessage("§l§a(!)§r §aYou have given " . $name . " a §l§bPvP Elixir§r§7.§r");
                   	...
                   	case "healer":
                      	$sender->sendMessage("§l§a(!)§r §aYou have given " . $name . " a §l§aHealer Elixir§r§7.§r");
                       	...
                       	break;
                   	case "mining":
                       	$sender->sendMessage("§l§a(!)§r §aYou have given " . $name . " a §l§dMining Elixir§r§7.§r");
                    ...
                    default:
                        ...
                        $sender->sendMessage("§l§6(!)§r §6Unknown type: $args[1]");
                        $sender->sendMessage("§l§6(!)§r §6Available types:§r"." ".implode("§r, ", $types));
                }
            } else {
        ...
        if($data !== null){
            ...
            if($data == "Reset"){
                ...
                if(!in_array($data, $tags)){
                    $player->sendMessage("§l§c»§r §7You dont have tag: " . $manager->tags[$data]);
                    return;
                }
            }
            $manager->setForceTag($player, $data);
            $player->sendMessage("§l§a»§r §7You've successfully set your tag.§r");
        }
    }
}
