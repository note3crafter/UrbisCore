...
class ListCommand extends Command {
    ...
    public function __construct() {
        parent::__construct("list", "List currently online players.");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        foreach($this->getCore()->getServer()->getOnlinePlayers() as $player) {
            ...
            if($player->getRank()->getIdentifier() >= Rank::ADVENTURER and $player->getRank()->getIdentifier() <= Rank::TITAN) {
                $players[] = $player->getRank()->getIdentifier() === Rank::ADVENTURER ? TextFormat::WHITE . $player->getName() : $player->getRank()->getColoredName() . " " . TextFormat::WHITE . $player->getName();
                continue;
            }
            if($player->getRank()->getIdentifier() >= Rank::NOBLE and $player->getRank()->getIdentifier() <= Rank::MERCENARY) {
            ...
            if($player->getRank()->getIdentifier() === Rank::YOUTUBER or $player->getRank()->getIdentifier() === Rank::FAMOUS or $player->getRank()->getIdentifier() === Rank::YOUTUBER_PLUS) {
