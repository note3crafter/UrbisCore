...
use core\libs\form\element\Toggle;
...
class TransactionForm extends CustomForm {
     ...
     */
    public function __construct(CorePlayer $player, PriceEntry $entry) {
        $this->priceEntry = $entry;
        $title = TextFormat::BOLD . TextFormat::WHITE . $entry->getName();
        $elements = [];
        $message = TextFormat::GRAY . "Your balance: " . TextFormat::DARK_GREEN . "$" . TextFormat::GREEN . number_format($player->getBalance());
        $elements[] = new Label("Balance", $message);
        $buyPrice = $entry->getBuyPrice();
        if($buyPrice === null) {
            $buyPrice = TextFormat::WHITE . "This item is not buyable!";
        }
        else {
    ...
    public function onSubmit(Player $player, CustomFormResponse $data): void {
        ...
        if($player->getBalance() < $amount) {
            $player->sendMessage("§l§c(!)§r §cYou don't have enough balance!");
            return;
        }
