...
class ProxyCheckTask extends AsyncTask {
    ...
    public function __construct(string $player, string $address, string $key) {
        ...
        $this->key = $key;
        Urbis::getInstance()->getLogger()->notice("Unknown ip detected in $player, checking for a vpn or proxy now.");
    }
    ...
    public function onCompletion(Server $server) {
        ...
        switch($result) {
            case 0:
                ...
                $stmt = Urbis::getInstance()->getMySQLProvider()->getDatabase()->prepare("INSERT INTO ipAddress(uuid, username, ipAddress, riskLevel) VALUES(?, ?, ?, ?)");
