...
class CoinFlipConfirmationForm extends ModalForm {
     ...
     *
     * @param CorePlayer $target
     */
    public function __construct(CorePlayer $target) {
        $this->target = $target;
    ...
    public function onSubmit(Player $player, bool $choice): void {
        ...
        if($choice == true) {
            if((!$this->target instanceof CorePlayer) or (!$this->target->isOnline())) {
            ...
            $winTotal = $amount * 2;
            $player->getServer()->broadcastMessage(TextFormat::GREEN . $winner->getName() . TextFormat::DARK_GRAY . " ($wins-$losses)" . TextFormat::GRAY . " has defeated " . TextFormat::RED . $loser->getName() . TextFormat::DARK_GRAY . " ($wins2-$losses2) " . TextFormat::GRAY . "in a " . TextFormat::LIGHT_PURPLE . "$". number_format($winTotal) . TextFormat::GRAY . " coin flip!");
