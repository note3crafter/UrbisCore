...
class FactionManager {
    ...
    public function init(): void {
        ...
        while($stmt->fetch()) {
            ...
            if($x !== null and $y !== null and $z !== null) {
                $home = new Position($x, $y, $z, Urbis::getInstance()->getServer()->getLevelByName(Faction::CLAIM_WORLD));
            }
    ...
    /**
     * @param string        $name
     * @param CorePlayer $leader
     *
     * @throws FactionException
     */
    public function createFaction(string $name, CorePlayer $leader): void {
        if(isset($this->factions[$name])) {
            throw new FactionException("Unable to override an existing faction!");
        }
        $members = $leader->getName();
        $stmt = Urbis::getInstance()->getMySQLProvider()->getDatabase()->prepare("INSERT INTO factions(name, members) VALUES(?, ?)");
     ...
     */
    public static function sendFactionMap(CorePlayer $player): void {
        ...
        for($x = $chunkX - 3; $x <= $chunkX + 3; $x++) {
            ...
            for($z = $chunkZ - 5; $z <= $chunkZ + 5; $z++) {
                ...
                if(($claim = Urbis::getInstance()->getFactionManager()->getClaimByHash(Level::chunkHash($x, $z))) !== null and $player->getLevel()->getName() === Faction::CLAIM_WORLD) {
        ...
        $claim = "None";
        if(($currentClaim = Urbis::getInstance()->getFactionManager()->getClaimInPosition($player)) !== null) {
