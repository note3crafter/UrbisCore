...
class Explosion {
    ...
    public const PERCENTAGE_CHANCE = 3;
    ...
    public function explodeB(): bool {
        ...
        $x = intval($source->getX());
        $y = intval($source->getY());
        $z = intval($source->getZ());
        ...
        foreach($this->affectedBlocks as $block) {
            ...
            elseif($yieldDrops = (mt_rand(0, 100) < $yield)) {
                foreach($block->getDrops($air) as $drop) {
                    if ($blockatid !== 9 && $blockatid !== 8 && $blockatid !== 11 && $blockatid !== 10){
                        $this->level->dropItem($block->add(0.5, 0.5, 0.5), $drop);
                    }else{
                        if ($chance < self::PERCENTAGE_CHANCE){
                            $this->level->dropItem($block->add(0.5, 0.5, 0.5), $drop);
                        }
                    }
                }
            }
            if ($blockatid === 9 or $blockatid === 8 or $blockatid === 11 or $blockatid === 10) {
                if ($chance < self::PERCENTAGE_CHANCE){
